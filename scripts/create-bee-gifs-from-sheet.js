...
const SHEET = path.join(OUT_DIR, 'bee-f4-spritesheet-preview.png');
const W = 96;
const H = 72;
...
function frameFromCell(sheet, col, row) {
  ...
  minX = Math.max(x0, minX - 8);
  minY = Math.max(y0, minY - 8);
  maxX = Math.min(x1 - 1, maxX + 8);
  maxY = Math.min(y1 - 1, maxY + 8);

  const srcW = maxX - minX + 1;
  const srcH = maxY - minY + 1;
  const scale = Math.min(88 / srcW, 64 / srcH);
  ...
  for (let y = 0; y < dstH; y++) {
    for (let x = 0; x < dstW; x++) {
      const sx = minX + Math.min(srcW - 1, Math.floor(x / scale));
      const sy = minY + Math.min(srcH - 1, Math.floor(y / scale));
      const p = (sy * sheet.width + sx) * 4;
      const r = sheet.rgba[p];
      const g = sheet.rgba[p + 1];
      const b = sheet.rgba[p + 2];
      const a = sheet.rgba[p + 3];
      out[(offY + y) * W + offX + x] = isKey(r, g, b, a) ? 0 : nearestIndex(r, g, b);
    }
  }
...
const sheet = parsePng(SHEET);
fs.writeFileSync(path.join(OUT_DIR, 'bee_fly.gif'), gif([0, 1, 2, 3].map((col) => frameFromCell(sheet, col, 0)), 12));
fs.writeFileSync(path.join(OUT_DIR, 'bee_read.gif'), gif([0, 1, 2, 3].map((col) => frameFromCell(sheet, col, 1)), 35));
fs.writeFileSync(path.join(OUT_DIR, 'bee_sleep.gif'), gif([0, 1, 2, 3].map((col) => frameFromCell(sheet, col, 2)), 45));
