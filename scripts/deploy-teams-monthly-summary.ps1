...
param(
  ...
  [string]$WorkflowName = 'ado-monthly-pr-summary-staging',
  [string]$EndpointUri = 'https://mango-wave-09cff3700.7.azurestaticapps.net/api/monthly-summary'
)

$ErrorActionPreference = 'Stop'
$PSNativeCommandUseErrorActionPreference = $true
$templatePath = Join-Path $PSScriptRoot '..\infra\teams-monthly-summary.bicep'
...
Write-Host 'Deploying the Teams monthly summary workflow in a disabled state...'
$deploymentArgs = @(
...
$settingsArgs = @(
  ...
  '--resource-group', $ResourceGroup,
  '--setting-names', "MONTHLY_SUMMARY_TOKEN=$summaryToken",
  '--only-show-errors',
  '--output', 'none'
)
az @settingsArgs

Write-Host 'Enabling the Teams monthly summary workflow...'
$enableArgs = @(
  'resource', 'update',
  '--resource-group', $ResourceGroup,
  '--name', $WorkflowName,
