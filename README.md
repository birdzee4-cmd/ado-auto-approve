...
| Deployment Archive | `/deploy-history.html` | ค้นหา คัดกรอง และดูประวัติการรัน Build & Deploy ย้อนหลังบนระบบ Staging โดยเข้าผ่านหน้า Activity |
...
Total matching apps: 1241
Running apps: 1016
Backend list response time: ~3 seconds
```
...
| `APP_SERVICE_ARM_REQUEST_TIMEOUT_SECONDS` | No | default `30`; timeout per Azure Resource Graph / ARM REST request |
| `APP_SERVICE_ENABLE_SLOW_LIST_FALLBACK` | No | default disabled; set `true` only for diagnostics to allow the slower ARM resources fallback when Resource Graph fails |
...
1. Enable System-Assigned Managed Identity ให้ Azure Function App `func-ado-auto-approve-appservice-api`
2. Assign RBAC ให้ Function App managed identity ไม่ใช่ SWA managed API identity
3. Production ปัจจุบันใช้ `Website Contributor` ที่ subscription scope เพื่อรองรับ `APP_SERVICE_RESOURCE_GROUP=ALL`
4. ถ้าต้องจำกัด scope ให้แคบลง ให้ assign role เฉพาะ resource groups หรือ selected apps ที่ต้องการให้ portal จัดการ และตั้ง `APP_SERVICE_RESOURCE_GROUP` ให้ตรงกับ scope นั้น
5. Preferred custom role actions:
   - `Microsoft.Web/sites/read`
   - `Microsoft.Web/sites/config/read`
   - `Microsoft.Web/sites/restart/action`
6. Configure Static Web Apps authentication roles: `it_support_approve`, `tester_appservice_manager`, `admin`
7. สร้าง SharePoint List ชื่อ `App Service Portal Log`

### App Service Portal backend implementation notes

- `/api/appservices` บน SWA เป็น proxy ไปยัง Function App เฉพาะ App Service Portal API
- Function App ใช้ Managed Identity ขอ ARM token และเรียก Azure Resource Graph สำหรับ list `stg-*` แบบ subscription-wide
- Resource Graph paging ต้องใช้ `$top` และ `$skipToken`; มี guard กัน repeated skip token และ page count เกินคาด เพื่อป้องกัน request ค้างจน proxy timeout
- Settings และ Restart ยังคงใช้ `@azure/arm-appservice` โดยอ้างอิง resource group จริงของ app ที่ได้จาก list
- GitHub Actions workflow `Azure Functions App Service Portal API CI/CD` deploy เฉพาะ `appservice-api` และ shared modules ที่ portal ใช้
